	...
	function onSelected(option: Option) {
		...
		if (selectedArray.length >= 1) {
			if (selectedArray.length < maxSelectable) {
				if (selectedArray.includes(selected.label) === false) {
					selectedArray = [...selectedArray, selected.label]
				}
			}
		} else selectedArray = [selected.label];
		toggle();
		search = '';
	}
	function removeSelected(e){
		dispatch('remove', e)
		let selectedItem = e.target.parentElement.textContent.trim();
		selectedArray = selectedArray.filter((array: string) => array !== `${selectedItem}`)
		console.log(e);
			...
			{#if selectMultiple}
				...
				<div class="flex gap-1 flex-wrap w-full">
					{#each selectedArray as selectedArrayElement}
						<p class="p-1 bg-[#f0f0f0] rounded">{selectedArrayElement}
							<iconify-icon icon="iconoir:cancel" on:click={(e) => removeSelected(e)}></iconify-icon>
						</p>
					{/each}
				</div>
	...
	{#if open}
		...
		>
			...
			{#each options as option}
				<li
					class="p-2 text-[14px] hover:bg-primary cursor-pointer hover:bg-[#A52921] hover:text-white
